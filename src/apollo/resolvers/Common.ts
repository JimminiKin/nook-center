...
export function createConnectionFromFullNodeEdgeList<TEntity extends Node>(
  ...
  if (afterCursor) {
    const afterCursorIndex = allPossibleEdges.findIndex(
      (node: Node): boolean => {
    ...
    if (afterCursorIndex === -1) {
      ...
      startSliceIndex = Math.min(
        afterCursorIndex + 1,
        allPossibleEdges.length - 1
      );
      endSliceIndex =
        startSliceIndex +
        Math.min(nbEdgesToReturn, allPossibleEdges.length - startSliceIndex);
    }
  }

  const edges: Edge<TEntity>[] = allPossibleEdges
