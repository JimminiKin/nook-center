import {Gender, Species, Personality, StarSign, RelationshipQuality} from '@gen/common/graphql';
...
export const relationShips = (personality: Personality): Record<Personality, RelationshipQuality> => {
...
const validateMapping = () => {
	Object.values(Personality).forEach((persoToCheck) => {
		...
		Object.keys(rel).forEach((subPerso) => {
			...
			const checkeeToChecked = relationShips(subPerso as Personality)[persoToCheck];
			const matched = checkedToCheckee == checkeeToChecked;
			const noNeutral =
				checkedToCheckee !== RelationshipQuality.Neutral && checkeeToChecked !== RelationshipQuality.Neutral;
			if (!matched) {
				console.log('\tto', subPerso);
				console.log('\t\t', `${persoToCheck} finds ${subPerso} ${checkedToCheckee}`);
				console.log('\t\t', `${subPerso} finds ${persoToCheck} ${checkeeToChecked}`);
			}
		});
	});
};
